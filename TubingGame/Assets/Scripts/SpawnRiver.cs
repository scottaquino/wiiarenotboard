	...
	void OnTriggerEnter2D(Collider2D col)
	{
		if (col.gameObject.tag == "Player" && !spawned) {
			...
			if(newChunk == riverBank[0])
				newPos = new Vector3 (transform.position.x + 13.15f, transform.position.y - (newChunk.GetComponent<SpriteRenderer> ().bounds.size.y / 2) - .2f, -1);
			else if(newChunk == riverBank[1])
				newPos = new Vector3 (transform.position.x - 10.0f, transform.position.y - (newChunk.GetComponent<SpriteRenderer> ().bounds.size.y / 2) - .2f, -1);
